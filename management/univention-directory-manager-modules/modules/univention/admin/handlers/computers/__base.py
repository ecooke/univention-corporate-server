...
class ComputerObject(univention.admin.handlers.simpleComputer, nagios.Support):
	...
	def rewrite_alias_filter(cls, filter, mapping, lo):
		if filter.variable == 'dnsAlias':
			found = univention.admin.handlers.dns.alias.lookup_alias_filter(lo, unicode(filter))
			if found:
				filter.transform_to_conjunction(univention.admin.filter.parse(found))
			else:
				# nothing matched! add a impossible filter here!
				filter.transform_to_conjunction(univention.admin.filter.parse('(&(objectClass=top)(!(objectClass=top)))'))
